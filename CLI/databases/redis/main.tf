...
module "redis01" {
  # UPDATE VERSION WHEN RELEASED
  # source = "git::ssh://git@github.com/oracle-devrel/terraform-oci-cloudbricks-redis?ref=v1.0.1"
  source = "github.com/oraclecloudbricks/terraform-oci-cloudbricks-redis?ref=er-orm"
  ...
  redis_replica_ad_count     = var.redis01_redis_replica_ad_count
  redis_replica_fd_count     = var.redis01_redis_replica_fd_count
